...
from models.yolov7.general import (check_requirements, clean_str,
                                   resample_segments, segment2box,
                                   segments2boxes, xyn2xy, xywh2xyxy,
                                   xywhn2xyxy, xyxy2xywh)
from models.yolov7.torch_utils import torch_distributed_zero_first
...
def sample_segments(img, labels, segments, probability=0.5):
    ...
    if probability and n:
        ...
        for j in random.sample(range(n), k=round(probability * n)):
            l, s = labels[j], segments[j]
            box = l[1].astype(int).clip(0, w - 1), l[2].astype(int).clip(
                0, h - 1), l[3].astype(int).clip(0,
                                                 w - 1), l[4].astype(int).clip(
                                                     0, h - 1)
