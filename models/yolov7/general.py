...
from models.yolov7.google_utils import gsutil_getsize
from models.yolov7.metrics import fitness
from models.yolov7.torch_utils import init_torch_seeds
...
def non_max_suppression_kpt(prediction,
                            conf_thres=0.25,
                            iou_thres=0.45,
                            classes=None,
                            agnostic=False,
    ...
    """
    if nc is None:
        nc = prediction.shape[2] - 5 if not kpt_label else prediction.shape[
            2] - 56  # number of classes
    xc = prediction[..., 4] > conf_thres  # candidates
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            ...
            if not kpt_label:
                ...
                x = torch.cat((box, conf, j.float(), kpts),
                              1)[conf.view(-1) > conf_thres]
...
def print_mutation(hyp, results, yaml_file='hyp_evolved.yaml', bucket=''):
    ...
    c = '%10.4g' * len(
        results
    ) % results  # results (P, R, mAP@0.5, mAP@0.5:0.95, val_losses x 3)
    print('\n%s\n%s\nEvolved fitness: %s\n' % (a, b, c))
