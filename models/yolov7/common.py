...
from models.yolov7.datasets import letterbox
from models.yolov7.general import (increment_path, make_divisible,
                                   non_max_suppression, scale_coords,
                                   xyxy2xywh)
from models.yolov7.plots import color_list, plot_one_box
from models.yolov7.torch_utils import time_synchronized
...
class RobustConv(nn.Module):
    ...
    def __init__(self,
        ...
        self.gamma = nn.Parameter(
            ...
            torch.ones(c2)) if layer_scale_init_value > 0 else None
...
class RobustConv2(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_deconv = nn.ConvTranspose2d(in_channels=c1,
                                              out_channels=c2,
                                              kernel_size=s,
                                              stride=s,
                                              padding=0,
...
class Stem(nn.Module):
    ...
    def forward(self, x):
        x = self.cv1(x)
        return self.cv4(torch.cat((self.cv3(self.cv2(x)), self.pool(x)),
                                  dim=1))
...
class Res(nn.Module):
    ...
    def forward(self, x):
        return x + self.cv3(self.cv2(self.cv1(x))) if self.add else self.cv3(
            self.cv2(self.cv1(x)))
...
class Detections:
    ...
    def save(self, save_dir='runs/hub/exp'):
        save_dir = increment_path(
            save_dir,
            exist_ok=save_dir != 'runs/hub/exp')  # increment save_dir
...
class ConvBN(nn.Module):
    ...
    def switch_to_deploy(self):
        ...
        conv = nn.Conv2d(in_channels=self.conv.in_channels,
...
class OREPA_3x3_RepConv(nn.Module):
    ...
    def __init__(self,
        ...
        if internal_channels_1x1_3x3 == in_channels:
            ...
            self.weight_rbr_1x1_kxk_conv1 = nn.Parameter(
                torch.Tensor(internal_channels_1x1_3x3,
                             int(in_channels / self.groups), 1, 1))
            nn.init.kaiming_uniform_(self.weight_rbr_1x1_kxk_conv1,
                                     a=math.sqrt(1.0))
        self.weight_rbr_1x1_kxk_conv2 = nn.Parameter(
            torch.Tensor(out_channels,
                         int(internal_channels_1x1_3x3 / self.groups),
                         kernel_size, kernel_size))
        nn.init.kaiming_uniform_(self.weight_rbr_1x1_kxk_conv2,
                                 a=math.sqrt(1.0))
        self.branch_counter += 1
        ...
        self.vector = nn.Parameter(
            torch.Tensor(self.branch_counter, self.out_channels))
    ...
    def weight_gen(self):
        ...
        weight_rbr_origin = torch.einsum('oihw,o->oihw',
                                         self.weight_rbr_origin,
                                         self.vector[0, :])

        weight_rbr_avg = torch.einsum(
            'oihw,o->oihw',
            torch.einsum('oihw,hw->oihw', self.weight_rbr_avg_conv,
                         self.weight_rbr_avg_avg), self.vector[1, :])

        weight_rbr_pfir = torch.einsum(
            'oihw,o->oihw',
            torch.einsum('oihw,ohw->oihw', self.weight_rbr_pfir_conv,
                         self.weight_rbr_prior), self.vector[2, :])

        weight_rbr_1x1_kxk_conv1 = None
        if hasattr(self, 'weight_rbr_1x1_kxk_idconv1'):
            weight_rbr_1x1_kxk_conv1 = (self.weight_rbr_1x1_kxk_idconv1 +
                                        self.id_tensor).squeeze()
        ...
        weight_rbr_gconv = self.dwsc2full(self.weight_rbr_gconv_dw,
                                          self.weight_rbr_gconv_pw,
                                          self.in_channels)
        weight_rbr_gconv = torch.einsum('oihw,o->oihw', weight_rbr_gconv,
                                        self.vector[4, :])
...
class RepConv_OREPA(nn.Module):
    ...
    def __init__(self,
        ...
        if deploy:
            ...
            ) if self.out_channels == self.in_channels and s == 1 else None
            self.rbr_dense = OREPA_3x3_RepConv(in_channels=self.in_channels,
                                               out_channels=self.out_channels,
                                               kernel_size=k,
                                               stride=s,
                                               padding=padding,
                                               groups=groups,
                                               dilation=1)
            self.rbr_1x1 = ConvBN(in_channels=self.in_channels,
                                  out_channels=self.out_channels,
                                  kernel_size=1,
                                  stride=s,
                                  padding=padding_11,
...
class SwinTransformerLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if min(
                H_, W_
        ) < self.window_size or H_ % self.window_size != 0 or W_ % self.window_size != 0:
            Padding = True
            # print(f'img_size {min(H_, W_)} is less than (or not divided by) window_size {self.window_size}, Padding.')
            pad_r = (self.window_size -
                     W_ % self.window_size) % self.window_size
            pad_b = (self.window_size -
                     H_ % self.window_size) % self.window_size
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, c1, c2, num_heads, num_layers, window_size=8):
        ...
        self.blocks = nn.Sequential(*[
            SwinTransformerLayer(dim=c2,
                                 num_heads=num_heads,
                                 window_size=window_size,
                                 shift_size=0 if (i %
                                                  2 == 0) else window_size //
...
class SwinTransformerLayer_v2(nn.Module):
    ...
    def __init__(self,
        ...
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp_v2(in_features=dim,
                          hidden_features=mlp_hidden_dim,
                          act_layer=act_layer,
                          drop=drop)
    ...
    def forward(self, x):
        ...
        attn_windows = attn_windows.view(-1, self.window_size,
                                         self.window_size, C)
        shifted_x = window_reverse_v2(attn_windows, self.window_size, H,
                                      W)  # B H' W' C
...
class SwinTransformer2Block(nn.Module):
    ...
    def __init__(self, c1, c2, num_heads, num_layers, window_size=7):
        ...
        # remove input_resolution
        self.blocks = nn.Sequential(*[
            SwinTransformerLayer_v2(dim=c2,
                                    num_heads=num_heads,
                                    window_size=window_size,
                                    shift_size=0 if (
