...
from models.yolov7.general import xywh2xyxy, xyxy2xywh
from models.yolov7.metrics import fitness
...
def plot_study_txt(path='',
    ...
    for f in sorted(Path(path).glob('study*.txt')):
        ...
        ax2.plot(y[6, 1:j],
                 ...
                 linewidth=2,
                 markersize=8,
                 label=f.stem.replace('study_coco_',
                                      '').replace('yolo', 'YOLO'))
...
def plot_labels(labels, names=(), save_dir=Path(''), loggers=None):
    ...
    for cls, *box in labels[:1000]:
        ImageDraw.Draw(img).rectangle(box,
                                      width=1,
...
def profile_idetection(start=0, stop=0, labels=(), save_dir=''):
    ...
    s = [
        'Images', 'Free Storage (GB)', 'RAM Usage (GB)', 'Battery',
...
def plot_results_overlay(
        start=0, stop=0):  # from utils.plots import *; plot_results_overlay()
    ...
    for sk_id, sk in enumerate(skeleton):
        ...
        if steps == 3:
            ...
            if conf1 < 0.5 or conf2 < 0.5:
                continue
        if pos1[0] % 640 == 0 or pos1[1] % 640 == 0 or pos1[0] < 0 or pos1[
                1] < 0:
            continue
        if pos2[0] % 640 == 0 or pos2[1] % 640 == 0 or pos2[0] < 0 or pos2[
                1] < 0:
            continue
