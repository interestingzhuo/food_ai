...
from models.yolov7.general import (bbox_alpha_iou, bbox_iou, box_ciou,
                                   box_diou, box_giou, box_iou, xywh2xyxy)
from models.yolov7.torch_utils import is_parallel
...
class SigmoidBin(nn.Module):
    ...
    def forward(self, pred):
        assert pred.shape[
            -1] == self.length, 'pred.shape[-1]=%d is not equal to self.length=%d' % (
    ...
    def training_loss(self, pred, target):
        assert pred.shape[
            -1] == self.length, 'pred.shape[-1]=%d is not equal to self.length=%d' % (
                pred.shape[-1], self.length)
        assert pred.shape[0] == target.shape[
            0], 'pred.shape=%d is not equal to the target.shape=%d' % (
        ...
        target_bins = torch.full_like(pred_bin, self.cn,
...
class RankSort(torch.autograd.Function):
    ...
    def forward(ctx, logits, targets, delta_RS=0.50, eps=1e-10):
        ...
        for ii in order:
            ...
            if delta_RS > 0:
                fg_relations = torch.clamp(fg_relations / (2 * delta_RS) + 0.5,
                                           min=0,
                                           max=1)
                bg_relations = torch.clamp(bg_relations / (2 * delta_RS) + 0.5,
                                           min=0,
                                           max=1)
            else:
            ...
            current_sorting_error = torch.sum(fg_relations *
                                              (1 - fg_targets)) / rank_pos
...
class aLRPLoss(torch.autograd.Function):
    ...
    def forward(ctx, logits, targets, regression_losses, delta=1., eps=1e-5):
        ...
        for ii in order:
            ...
            fg_relations = torch.clamp(fg_relations / (2 * delta) + 0.5,
                                       min=0,
                                       max=1)
            ...
            bg_relations = torch.clamp(bg_relations / (2 * delta) + 0.5,
                                       min=0,
                                       max=1)
            ...
            if FP_num > eps:
                fg_grad[ii] = -(torch.sum(fg_relations * regression_losses) +
                                FP_num) / rank[ii]
...
class ComputeLossBinOTA:
    ...
    def __init__(self, model, autobalance=False):
        ...
        wh_bin_sigmoid = SigmoidBin(bin_count=self.bin_count,
                                    min=0.0,
                                    max=4.0,
                                    use_loss_regression=False).to(device)
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            obj_idx = self.wh_bin_sigmoid.get_length(
            ) * 2 + 2  # x,y, w-bce, h-bce     # xy_bin_sigmoid.get_length()*2
            ...
            if n:
                ...
                pbox = torch.cat((px.unsqueeze(1), py.unsqueeze(1),
                                  pw.unsqueeze(1), ph.unsqueeze(1)),
                                 1).to(device)  # predicted box
                ...
                selected_tcls = targets[i][:, 1].long()
                if self.nc > 1:  # cls loss (only if multiple classes)
                    t = torch.full_like(ps[:, (1 + obj_idx):],
                                        self.cn,
                                        device=device)  # targets
                    t[range(n), selected_tcls] = self.cp
    ...
    def build_targets(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            for i, pi in enumerate(p):
                ...
                pw = self.wh_bin_sigmoid.forward(
                    fg_pred[..., 2:(3 + self.bin_count)].sigmoid(
                    )) * anch[i][idx][:, 0] * self.stride[i]
                ph = self.wh_bin_sigmoid.forward(
                    fg_pred[..., (3 + self.bin_count):obj_idx].sigmoid(
                    )) * anch[i][idx][:, 1] * self.stride[i]

                pxywh = torch.cat([pxy, pw.unsqueeze(1),
                                   ph.unsqueeze(1)],
                                  dim=-1)
                pxyxy = xywh2xyxy(pxywh)
                pxyxys.append(pxyxy)
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model
        ...
        for i in range(self.nl):  # layer index, layer predictions
            ...
            b_aux, a_aux, gj_aux, gi_aux = bs_aux[i], as_aux_[i], gjs_aux[
                i], gis_aux[i]  # image, anchor, gridy, gridx
            tobj = torch.zeros_like(pi[..., 0], device=device)  # target obj
            tobj_aux = torch.zeros_like(pi_aux[..., 0],
            ...
            if n_aux:
                ps_aux = pi_aux[
                    b_aux, a_aux, gj_aux,
                    gi_aux]  # prediction subset corresponding to targets
                ...
                if self.nc > 1:  # cls loss (only if multiple classes)
                    t_aux = torch.full_like(ps_aux[:, 5:],
                                            self.cn,
                                            device=device)  # targets
            ...
            lobj += obji * self.balance[i] + 0.25 * obji_aux * self.balance[
                i]  # obj loss
            if self.autobalance:
                self.balance[i] = self.balance[
                    i] * 0.9999 + 0.0001 / obji.detach().item()
