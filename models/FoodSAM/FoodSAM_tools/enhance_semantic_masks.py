...
def predict_sam_label(data_folder, category_txt,
    ...
    for test_path, category_list in zip(data_folder, category_lists):
        ...
        #for img_id in img_ids:
            #print('img_id',img_id)
        mask_file_path = os.path.join(test_path, masks_path_name)
        pred_mask_file_path = os.path.join(test_path, pred_mask_file_name)
...
def enhance_masks(img_name,data_folder, img_path,category_txt, color_list_path, num_class=104, area_thr=0, ratio_thr=0.5, top_k=80,
                  masks_path_name="sam_mask/masks.npy",
                  ...
                  sam_mask_label_file_dir='sam_mask_label'):
    data_folder = os.path.join(data_folder,img_name)
    ...
    #    continue
    category_info_path = os.path.join(data_folder, sam_mask_label_file_dir, new_mask_label_file_name)
    sam_mask_folder = os.path.join(data_folder)
    pred_mask_path = os.path.join(data_folder, pred_mask_file_name)
    #img_path = '/mnt/data/chaoran/food_ai_cv/data/1000120.jpg'#os.path.join(data_folder, img_folder, '1000120.jpg')
    save_dir = os.path.join(data_folder)
